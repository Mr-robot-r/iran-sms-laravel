...
final class MedianaDriverTest extends TestCase
{
    #[Test]
    ...
    public function it_throws_exception_if_we_pass_non_key_value_pairs_to_send_with_pattern(): void
    {
        ...
        $this->expectExceptionMessage('Provider "mediana" only accepts pattern data as key-value pairs.');
    ...
    public function it_throws_an_exception_for_sending_otp(): void
    {
        ...
        $this->expectExceptionMessage('Provider "mediana" does not support sending "otp" message, please use "pattern" method instead.');
    ...
    private function driver(): MedianaDriver
    {
        Config::set('iran-sms.providers.mediana.token', 'sms_token');
        Config::set('iran-sms.providers.mediana.from', '123');

        return $this->app->make(MedianaDriver::class);
    }
}
